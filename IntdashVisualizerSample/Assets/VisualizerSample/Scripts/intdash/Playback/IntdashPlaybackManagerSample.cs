...
public class IntdashPlaybackManagerSample : MonoBehaviour
{
    ...
    public int RequestPartialDataDuration = 15;
    ...
    private void OnDestroy()
    {
        ...
        if (Shared == this)
            Shared = null;
        bulkRequestId = Guid.NewGuid();
        partialRequestId = Guid.NewGuid();
    }
    ...
    public PlaybackStatus Status { private set; get; } = PlaybackStatus.Stop;
    ...
    public delegate void SeekedListener(float value, long time);
    ...
    private void InvokeSeekedEvent(float value, long time)
    {
        ...
        ElapsedTime = value;
        partialDataRequestedTime = time;
        Dispatcher.RunOnMainThread(() =>
        {
            OnSeeked?.Invoke(value, time);
        });
    }
    ...
    public void Seek(float value)
    {
        ...
        InvokeSeekedEvent(time, StartTimeTicks + time.SecondsToTicks());
    }
    ...
    void Update()
    {
        ...
        if (!(Status == PlaybackStatus.Play)) return;
    ...
    private void CheckRequestPartialData()
    {
        ...
        if (!(CurrentTimeTicks > partialDataRequestedTime - RequestPartialDataDurationTicks)) return;
        RequestPartialData(partialDataList, false, this.partialRequestId, partialDataRequestedTime, EndTimeTicks, this.partialRequestCancellationTokenSource.Token);
    }
    ...
    public bool SetPlaybackTime(DateTime startTime, DateTime endTime)
    {
        ...
        this.ElapsedTime = 0;
        this.partialDataRequestedTime = 0;
    ...
    {
        Debug.Log("StartRequestFirstData()");
        ...
        Task.Run(() =>
        {
            ...
            this.bulkRequestId = reqId;
            this.partialRequestId = reqId;
            var cancellationTokenSource = new CancellationTokenSource();
            this.bulkRequestCancellationTokenSource = cancellationTokenSource;
            this.partialRequestCancellationTokenSource = cancellationTokenSource;
            var requestCnt = 0;
            ...
            if (bulkPlaybacks.Count > 0)
            {
                ...
                RequestBulkData(dataList, true, reqId, StartTimeTicks, EndTimeTicks, cancellationTokenSource.Token, firstRequestCompletion);
            }
            ...
            if (partialPlaybacks.Count > 0)
            {
                ...
                RequestPartialData(partialDataList, true, reqId, StartTimeTicks, EndTimeTicks, cancellationTokenSource.Token, firstRequestCompletion);
            }
        });
    }
    ...
    private void RequestBulkData(Dictionary<string, List<Filter>> dataList, bool isFirst, Guid reqId, long time, long endTime, CancellationToken cancellationToken, Action firstRequestCompletion = null)
    {
        ...
        {
            Debug.Log($"RequestBulkData completed.");
            return;
        }
        ...
        Debug.Log($"RequestBulkData(time: {start.ToString("yyyy/MM/dd HH:mm:ss.ffffff")}, duration: {duration.TicksToSeconds()})");
        ...
        foreach (var kv in dataList)
        {
            foreach (var f in kv.Value)
            {
                Task.Run(async () =>
                {
                    try
                    {
                        var dataPoints = await RequestListDataPoints(kv.Key, new List<string> { f.Content }, start, end, cancellationToken: cancellationToken).ConfigureAwait(false);
                        if (reqId != this.bulkRequestId) return;
                    ...
                    {
                        Debug.LogError($"RequestListDataPoints error. {e.Message}, start: {start.ToString("yyyy-MM-ddTHH:mm:ss.FFFFFFZ")}, end: {end.ToString("yyyy-MM-ddTHH:mm:ss.FFFFFFZ")}");
                        completion.Invoke();
                    }
                });
            }
    ...
    private void RequestPartialData(Dictionary<string, List<Filter>> dataList, bool isFirst, Guid reqId, long time, long endTime, CancellationToken cancellationToken, Action firstRequestCompletion = null)
    {
        ...
        if (time >= endTime)
        {
            //Debug.Log($"RequestPartialData completed.");
            return;
        }

        isPartialRequesting = true;

        var duration = RequestPartialDataDurationTicks;
        if (endTime - time < duration)
        {
            duration = endTime - time;
        }
        ...
        var start = new DateTime(time, DateTimeKind.Utc);
        var end = new DateTime(time + duration, DateTimeKind.Utc);
        Debug.Log($"RequestPartialData(time: {start.ToString("yyyy/MM/dd HH:mm:ss.ffffff")}, duration: {duration.TicksToSeconds()})");
